...
export default function AdminDashboardClient() {
  ...
  useEffect(() => {
    ...
    async function loadInventory() {
      try {
        const response = await fetch('/api/v1/products?limit=100', { cache: 'no-store' });
        const payload = (await response.json()) as {
          data?: Array<{ name: string; stock: number; lowStockThreshold: number; vendor?: { storeName?: string } }>;
        };
        ...
        const rows = (payload.data ?? []).map((item) => ({
          name: item.name,
          vendor: item.vendor?.storeName || 'Unknown vendor',
          stock: item.stock,
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-3">
          ...
          {!loading && filteredInventory.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600">
              No live inventory found for this filter.
            </div>
          ) : null}
        </div>
      </div>
